...
PropertiesPanel.prototype._updateActivation = function (current) {
  ...
  updateLabel(panelNode, '[data-label-id]', element.businessObject.name || ''); //changed from id to name as per the requirement
};
  ...
  forEach(tabs, function (tab, tabIndex) {
    ...
    forEach(groups, function (group) {
      ...
      var groupNode = domify('<div class="djs-properties-group" data-group="' + group.id + '">' +
        ...
        '<span class="group-label"><span>' + group.label + '</span>' +
        '<span class="group-toggle"><i class="material-icons">keyboard_arrow_up</i></span></span>' +
      ...
      forEach(group.entries, function (entry) {
        ...
        groupNode.querySelector('.group-content').appendChild(entryNode);
