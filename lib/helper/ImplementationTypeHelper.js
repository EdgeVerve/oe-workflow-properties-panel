...
ImplementationTypeHelper.isEVFServiceTaskLike = function(element) {
  return is(element, 'camunda:ServiceTaskLike');
};

ImplementationTypeHelper.isFTServiceTaskLike = function(element) {
  return is(element, 'camunda:ServiceTaskLike');
};
 ...
 * @return {ModdleElement} the 'camunda:ServiceTaskLike' business object
 */
ImplementationTypeHelper.getEVFServiceTaskLikeBusinessObject = function(element) {
  ...
  return this.isEVFServiceTaskLike(element) && getBusinessObject(element);

};


ImplementationTypeHelper.getFTServiceTaskLikeBusinessObject = function(element) {
  ...
  return this.isFTServiceTaskLike(element) && getBusinessObject(element);
...
//  //   console.log(connectors) && connectors[0].ModdleElement.$type==='camunda:EVFConnector
//     if (typeof connectors !== 'undefined') {
//       return 'connector';
//     }else {
//       connectors = extensionsElementHelper.getExtensionElements(bo, 'camunda:EVFConnector');
//       if (typeof connectors !== 'undefined') {
//       return 'evfconnector';
//     }
//     }
...
var bo = this.getEVFServiceTaskLikeBusinessObject(element);
  if (this.isEVFServiceTaskLike(bo)) {
    var connectors = extensionsElementHelper.getExtensionElements(bo, 'camunda:EVFConnector');
    if (typeof connectors !== 'undefined') {
     // console.log("evfconnector")
      
      return 'evfconnector';
    }
  }

  var bo = this.getFTServiceTaskLikeBusinessObject(element);
  if (this.isFTServiceTaskLike(bo)) {
    var connectors = extensionsElementHelper.getExtensionElements(bo, 'camunda:FinalizeTransactionConnector');
    if (typeof connectors !== 'undefined') {
     // console.log("evfconnector")
