...
var selectbox = function(options, defaultParameters) {
  ...
  resource.html =
    ...
    '<select id="camunda-' + resource.id + '-select" class="style-scope work-flow" name="' + modelProperty + '"' +
    (canBeDisabled ? 'data-show="isDisabled" ' : '') + ' data-value>';

  forEach(selectOptions, function(option){
    resource.html += '<option class="style-scope work-flow" value="' + option.value + '">' + (option.name || '') + '</option>';
  });
  ...
  resource.cssClasses = ['dropdown','style-scope', 'work-flow'];
