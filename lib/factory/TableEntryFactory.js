...
var TABLE_ROW_DIV_SNIPPET = '<div class="pp-field-wrapper input-field pp-table-row style-scope work-flow">';
var DELETE_ROW_BUTTON_SNIPPET = '<button class="clear style-scope work-flow" data-action="deleteElement">' +
  '<span>X</span>' +
  '</button>';
...
function createInputRowTemplate(properties, canRemove, labels) {
  var template = TABLE_ROW_DIV_SNIPPET;
  template += createInputTemplate(properties, canRemove, labels);
  template += canRemove ? DELETE_ROW_BUTTON_SNIPPET : '';
  template += '</div>';
...
function createInputTemplate(properties, canRemove) {
  ...
  forEach(properties, function (prop) {
    template += '<input class="style-scope work-flow pp-table-row-columns-' + columns + ' style-scope work-flow' +
...
function createInputTemplate(properties, canRemove, labels) {
  var columns = properties.length;
  var template = '';
  forEach(properties, function (prop, key) {
    if (labels && labels[key]) {
      template += '<label for="camunda-table-row-cell-input-value" class="style-scope work-flow pp-table-row-columns-' + columns + '">' + labels[key] + '</label>';
...
function createLabelTemplate(labels) {
  ...
  forEach(labels, function (label) {
    template += '<label  class="style-scope work-flow pp-table-row-columns-' + columns + '">' + label + '</label>';
  });
  return ''; //template;
}
...
module.exports = function (options) {
  ...
  var factory = {
    id: id,
    html: (canAdd ?
        '<div class="style-scope work-flow pp-table-add-row" ' + (canBeShown ? 'data-show="show"' : '') + '>' +
        '<label>' + addLabel + '</label>' +
        '<button class="style-scope work-flow add" data-action="addElement"><span>+</span></button>' +
        '</div>' : '') +
      '<div class="style-scope work-flow pp-table" data-show="showTable">' +
      '<div class="style-scope work-flow pp-field-wrapper input-field pp-table-row">' +
      labelRow +
      '</div>' +
    ...
    createListEntryTemplate: function (value, index, selectBox) {
      if (labels) {
        return createInputRowTemplate(modelProperties, canRemove, labels);
      }
    ...
    addElement: function (element, node, event, scopeNode) {
      ...
      if (labels) {
        // alert(labels)
        template = domify(createInputRowTemplate(modelProperties, canRemove, labels));
      } else {
        template = domify(createInputRowTemplate(modelProperties, canRemove));
      }
