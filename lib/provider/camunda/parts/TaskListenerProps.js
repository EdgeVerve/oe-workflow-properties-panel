...
function createListenerTemplate(id) {
  return '<div class="style-scope work-flow djs-listener-area" data-scope>' +
    '<button class="style-scope work-flow clear" data-action="removeListener"><span>X</span></button>' +

    '<div class="style-scope work-flow pp-row">' +
    '<label for="cam-task-listener-event-type-' + id + '">Event Type</label>' +
    '<div class="style-scope work-flow pp-field-wrapper">' +
    '<select id="cam-task-listener-event-type-' + id + '" class="style-scope work-flow" name="eventType" data-value>' +
    /* '<option value="create">create</option>' +
     '<option value="assignment">assignment</option>' +
     '<option value="complete">complete</option>' +
     '<option value="delete">delete</option>' +*/
    '<option value="start">create</option>' +
    '<option value="end">complete</option>' +
    '</select>' +
    '</div>' +
    '</div>' +
    ...
    '<div class="style-scope work-flow pp-field-wrapper">' +
    '<select id="cam-task-listener-type-' + id + '" class="style-scope work-flow" name="listenerType" data-value>' +
    ...
    '</label>' +
    '<div class="style-scope work-flow pp-field-wrapper">' +
    '<input id="cam-task-listener-val-' + id + '" class="style-scope work-flow" type="text" name="listenerValue" />' +
    '<button class="style-scope work-flow clear" data-action="clearListenerValue" data-show="canClearListenerValue">' +
    '<span>X</span>' +
    '</button>' +
    '</div>' +
    '</div>' +
...
module.exports = function (group, element, bpmnFactory) {
  ...
  if (is(element, 'bpmn:UserTask') || is(element, 'bpmn:Task')) {
  ...
  group.entries.push({
    ...
    label: 'Listener',
    html: '<div class="style-scope work-flow cam-add-listener">' +
    '<label for="addTaskListener">Add Task Listener </label>' +
    '<button class="style-scope work-flow add" id="addTaskListener" data-action="addListener"><span>+</span></button>' +
    '</div>' +
    ...
    addListener: function (element, inputNode) {
      ...
      if(taskData[element.id]==undefined){
      taskData[element.id] = {};
      taskData[element.id].listenerIndex =  Array(); 
      }
      lastIdx++;
      var template = domify(createListenerTemplate(lastIdx));
      
       taskData[element.id].listenerIndex.push(lastIdx);
      listenerContainer.appendChild(template);
      if (listenerContainer.childElementCount >= 2) {
        document.getElementById('addTaskListener').style.display = 'none';
      } else {
        document.getElementById('addTaskListener').style.display = 'inline-block';
      }
      return true;
    },
    ...
    removeListener: function (element, entryNode, btnNode, scopeNode) {
      ...
      var listenerContainer = domQuery('[data-list-entry-container]', entryNode);
      if (listenerContainer.childElementCount >= 2) {
        document.getElementById('addTaskListener').style.display = 'none';
      } else {
        document.getElementById('addTaskListener').style.display = 'inline-block';
    ...
    cssClasses: ['pp-textfield', 'style-scope', 'work-flow']
  });
