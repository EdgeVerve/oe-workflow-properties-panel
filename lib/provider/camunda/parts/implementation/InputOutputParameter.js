...
function isScript1(elem) {
  return currentParamType === 'outgoingFlow' || currentParamType === 'dataObject';
...
function isOutGoingFlow() {
  return currentParamType === 'outgoingFlow';
}
...
module.exports = function (element, bpmnFactory, options) {
 ...
 /* if (element.type.includes('Task')) {
    ...
    entries.push(entryFactory.selectBox({
      id: idPrefix + 'listener',
      label: 'Listener',
      selectOptions: listenerSelectOptions,
      modelProperty: 'eventType',

      get: function (element, node) {
        var temp = getBusinessObject(element);
        //console.dir("======?>>>>");
        //console.dir(temp.id + "    ");
        var parameterType = "create";
        var inputField;
        if (temp.id !== undefined && temp.id.includes('Task')) {
          if (window.currentIO == "input")
            inputField = document.getElementById('cam-extensionElements-inputs');
          else
            inputField = document.getElementById('cam-extensionElements-outputs');

          //console.log('inputField: ' + inputField);
          if (inputField !== null) {
            var inputId = inputField.value;

            if (window.listenerData[inputId] === undefined || inputId == "" || window.listenerData[inputId] === "") {

              window.listenerData[inputId] = 'create';


            }

            parameterType = window.listenerData[inputId];
            document.getElementById('camunda-listener-select').value = parameterType;
          }
        }
        var bo = getSelected(element, node);
        if (bo !== undefined) {
          var inputParams = bo.$parent.inputParameters;
          for (var i = 0; i < inputParams.length; i++) {
            if (inputParams[i].eventType === undefined) {
              inputParams[i].eventType = 'create';
            }
          }
        }
        var values = {
        ...
        //cmdHelper.updateBusinessObject(element, bo, values);
     ...
     *1/
      ...
      set: function (element, values, node) {
        ...
        //console.dir(temp.id + "    ");
        var bo = getSelected(element, node);
        var parameterType = values.eventType;
        var properties = {
          value: undefined,
        ...
        if (temp.id !== undefined && temp.id.includes('Task')) {
          ...
          if (inputField !== null) {
            ...
            if (window.listenerData[inputId] === undefined || inputId == "" || window.listenerData[inputId] === '') {
              ...
              window.listenerData[inputId] = values.eventType;
            }
          }
        }
        currentParamType = values.eventType;
...
/*
  var assignmentType = [{
    ...
    // { value: 'incomingFlow', name: 'incomingFlow' },
    // { value: 'scriptOutput', name: 'scriptOutput' },
    // { value: 'dataObject ', name: 'dataObject ' },
    // { value: 'serviceResponse', name: 'serviceResponse' },
    // { value: 'outgoingFlow', name: 'outgoingFlow' },
    // { value: 'dataObject', name: 'dataObject' },
    // { value: 'serviceResponse', name: 'serviceResponse' }
  ];
  ...
  entries.push(entryFactory.selectBox({
    ...
    selectOptions: assignmentType,
    modelProperty: 'assignmentType',
    ...
    get: function (element, node) {
      ...
      if (temp.id !== undefined && temp.id.includes('Task')) {
        ...
        if (inputField !== null) {
          var inputId = inputField.value;
          if (window.assignmentTypeData[inputId] === undefined || inputId == "" || window.assignmentTypeData[inputId] === "") {
            window.assignmentTypeData[inputId] = ' ';
          }
          parameterType = window.assignmentTypeData[inputId];
          document.getElementById('camunda-assignmentType-select').value = parameterType;
        }
      }
      var bo = getSelected(element, node);
      if (bo !== undefined) {
        var inputParams = bo.$parent.inputParameters;
        for (var i = 0; i < inputParams.length; i++) {
          if (inputParams[i].assignmentTypeData == undefined) {
            inputParams[i].assignmentTypeData = ' ';
          }
        }
      }
      var values = {
        assignmentType: parameterType
      };
      return {
        assignmentType: parameterType
      };
    ...
    set: function (element, values, node) {
      ...
      //console.dir(temp.id + "    ");
      var bo = getSelected(element, node);
      var parameterType = values.assignmentType;
      var properties = {
        value: undefined,
        definition: undefined
      };
      ...
      if (temp.id !== undefined && temp.id.includes('Task')) {
        ...
        if (inputField !== null) {
          ...
          if (window.assignmentTypeData[inputId] === undefined || inputId == "" || window.assignmentTypeData[inputId] === '') {
            ...
            window.assignmentTypeData[inputId] = values.assignmentType;
          }
        }
      }
      var properties = {
      ...
      currentParamType = parameterType;

      console.log("Parameter Type:=== " + parameterType);
      if (parameterType === 'outgoingFlow' || parameterType === 'dataObject') {
        properties.definition = createParameterTypeElem('camunda:Script');
        var temp = cmdHelper.updateBusinessObject(element, bo, properties);
        //console.log('temp: ');
        //console.log(temp);
        temp.context.businessObject.assignmentType = parameterType;
        temp.context.properties.assignmentType = parameterType;
        return temp;
...
*1/
      var temp = cmdHelper.updateBusinessObject(element, bo, values);
      //console.log('temp: ');
      //console.log(temp);
      if (temp.context.businessObject.definition !== undefined)
        delete temp.context.businessObject.definition;
      return temp;
      //return cmdHelper.updateBusinessObject(element, bo, values);
    },
  ...
  //if(currentParamType === 'outgoingFlow'){
  entries.push({
    id: idPrefix + 'parameterType-script1',
    html: '<div  data-show="isScript1">' +
      script.template +
      '</div>',
    get: function (element, node) {
      var bo = getSelected(element, node);
      return (currentParamType === 'outgoingFlow' || currentParamType === 'dataObject') && bo && isScript(bo.definition) ? script.get(element, bo.definition) : {};
    },
    ...
    isScript1: function (element, node) {
      var bo = getSelected(element, node);
      return (currentParamType === 'outgoingFlow' || currentParamType === 'dataObject') && bo && isScript(bo.definition);
    },
