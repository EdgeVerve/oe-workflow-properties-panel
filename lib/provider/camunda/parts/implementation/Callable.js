...
module.exports = function (element, bpmnFactory, options) {
  ...
  var diagramDetails = {
    async: false,
    url: '/api/WorkflowDefinitions?filter={"where":{"latest":"true"}}',
    type: 'GET',
    error: function () {
    ...
    success: function (response) {
      ...
      for ( var opt in response){
        options.push({"name" : response[opt].name, "value": response[opt].name});
      }
    }
  }
  ...
  entries.push(entryFactory.comboBox({
    ...
    customValue: 'Custom Workflow',
    customName: 'Custom Workflow',
...
return entries;
  ...
  //   id : 'callable-element-ref',
  //   label: 'Called Element',    
  //   selectOptions: [ { name: 'LoanWorkflow', value: 'loanworkflow' }, { name: 'LeaveWorkflow', value: 'leaveWorkflow' } ],
  //   modelProperty: 'callableElementRef',
