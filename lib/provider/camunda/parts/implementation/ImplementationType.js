...
var EVCONNECTOR_OPTION = [{
  value: 'evfconnector',
  name: 'EVF Connector'
}];
//finalizetransactionconnectorTab
...
module.exports = function (element, bpmnFactory, options) {
  ...
  var hasDmnSupport = options.hasDmnSupport,
    ...
    hasEVServiceTaskLikeSupport = options.hasEVServiceTaskLikeSupport,
    hasFTServiceTaskLikeSupport = options.hasFTServiceTaskLikeSupport,
  ...
  if (hasServiceTaskLikeSupport) {
    selectOptions1 = selectOptions1.concat(CONNECTOR_OPTION);
  }
  // console.log("hasEVServiceTaskLikeSupport======>>" + hasEVServiceTaskLikeSupport)
  if (hasEVServiceTaskLikeSupport) {
    selectOptions1 = selectOptions1.concat(EVCONNECTOR_OPTION);
  }

  if(hasFTServiceTaskLikeSupport){
  ...
  entries.push(entryFactory.selectBox({
    ...
    set: function (element, values, node) {
      ...
      if (hasServiceTaskLikeSupport && newType === 'connector') {

        var connectors = extensionElementsHelper.getExtensionElements(bo, 'camunda:Connector');
        var tempConnectors = extensionElementsHelper.getExtensionElements(bo, 'camunda:EVFConnector');

        //  extensionElementsHelper.removeEntry(bo, element, tempConnectors);
        if (tempConnectors) {
          commands.push(map(tempConnectors, function (connector) {
        ...
        if (newType === 'connector') {
          ...
          if (!extensionElements || oldType !== 'connector') {
      ...
      if (hasEVServiceTaskLikeSupport && newType === 'evfconnector') {
        commands = [];
        commands.push(cmdHelper.updateBusinessObject(element, bo, props));
        var connectors = extensionElementsHelper.getExtensionElements(bo, 'camunda:EVFConnector');
        var tempConnectors = extensionElementsHelper.getExtensionElements(bo, 'camunda:Connector');
        ...
        if (newType === 'evfconnector') {


          var extensionElements = null; //bo.get('extensionElements');
          // console.log(2);
          ...
          var connector = elementHelper.createElement('camunda:EVFConnector', {}, extensionElements, bpmnFactory);
          commands.push(cmdHelper.addAndRemoveElementsFromList(
            element,
            extensionElements,
            'values',
      ...
      if (hasFTServiceTaskLikeSupport && newType === 'finalizetransactionconnector') {
        commands = [];
        commands.push(cmdHelper.updateBusinessObject(element, bo, props));
        var connectors = extensionElementsHelper.getExtensionElements(bo, 'camunda:FinalizeTransactionConnector');
        var tempConnectors = extensionElementsHelper.getExtensionElements(bo, 'camunda:Connector');
        var temp1Connectors = extensionElementsHelper.getExtensionElements(bo, 'camunda:EVFConnector');
        ...
        if (temp1Connectors) {
          commands.push(map(temp1Connectors, function (connector) {
            return extensionElementsHelper.removeEntry(bo, element, connector);
          }));
        }
        //   extensionElementsHelper.removeEntry(bo, element, tempConnectors);
        ...
        if (newType === 'finalizetransactionconnector') {
          ...
          var connector = elementHelper.createElement('camunda:FinalizeTransactionConnector', {}, extensionElements, bpmnFactory);
          commands.push(cmdHelper.addAndRemoveElementsFromList(
            element,
            extensionElements,
            'values',
