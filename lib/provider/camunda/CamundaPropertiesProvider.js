...
var EVFConnectorDetails = require('./parts/EVFConnectorDetailProps');
var FTConnectorDetails = require('./parts/FinalizeTransactionDetailProps')
// properties
...
function createEVFConnectorTabGroups(element, bpmnFactory, elementRegistry) {
  var EVFconnectorDetailsGroup = {
    id: 'evfconnector-details',
    label: 'Details',
    entries: []
  };
  EVFConnectorDetails(EVFconnectorDetailsGroup, element, bpmnFactory);
  return [
    EVFconnectorDetailsGroup
  ];
}


function createFTConnectorTabGroups(element, bpmnFactory, elementRegistry) {
  var FTconnectorDetailsGroup = {
    id: 'ftconnector-details',
    label: 'Details',
    entries: []
  };
  FTConnectorDetails(FTconnectorDetailsGroup, element, bpmnFactory);
  return [
    FTconnectorDetailsGroup
  ];
}
...
function CamundaPropertiesProvider(eventBus, bpmnFactory, elementRegistry, elementTemplates) {
  ...
  this.getTabs = function (element) {
    ...
    var evfconnectorTab = {
      id: 'evfconnector',
      label: 'EVF Connector',
      groups: createEVFConnectorTabGroups(element, bpmnFactory, elementRegistry),
      enabled: function (element) {
        var bo = implementationTypeHelper.getServiceTaskLikeBusinessObject(element);
        return bo && implementationTypeHelper.getImplementationType(bo) === 'evfconnector';
      }

    };
    var finalizetransactionconnectorTab = {
      id: 'finalizetransactionconnector',
      label: 'Finalize Transaction Connector',
      groups: createFTConnectorTabGroups(element, bpmnFactory, elementRegistry),
      enabled: function (element) {
        var bo = implementationTypeHelper.getServiceTaskLikeBusinessObject(element);
